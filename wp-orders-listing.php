...
class RJ_IndiaPost_Orders_List {
     ...
     */
    public function add_order_tracking_column_content($column, $order_id_or_order) {
        if ($column === 'order_tracking') {
            ...
            if (is_object($order_id_or_order) && is_a($order_id_or_order, 'WC_Order')) {
                // If we get a WC_Order object (HPOS)
                $order_id = $order_id_or_order->get_id();
                
                // Get tracking number via WC_Order method if available
                if (method_exists($order_id_or_order, 'get_meta')) {
                    $tracking_number = $order_id_or_order->get_meta('_rj_indiapost_tracking_number', true);
                } else {
                    $tracking_number = get_post_meta($order_id, '_rj_indiapost_tracking_number', true);
                }
            } else {
                // If we get an order ID (traditional post)
    ...
    public function save_tracking_from_list() {
        ...
        // Always save using post meta - it will work for both traditional and HPOS
        // This is because WooCommerce syncs post meta with HPOS tables automatically
